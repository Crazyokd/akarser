...
static lol_context_t* lol_context() {
    return &self;
}
...
static void lol_yaml_iter_init(lol_yaml_iter_t *iter, yaml_document_t *document) {
...
static int lol_yaml_iter_next(lol_yaml_iter_t *iter) {
    if (iter->node->type == YAML_MAPPING_NODE) {
        if (iter->pair) {
            iter->pair++;
...
static const char* lol_yaml_iter_key(lol_yaml_iter_t *iter) {
...
static void lol_yaml_iter_recurse(lol_yaml_iter_t *parent, lol_yaml_iter_t *iter) {
    memset(iter, 0, sizeof(lol_yaml_iter_t));
...
static const char *lol_yaml_iter_value(lol_yaml_iter_t *iter)
{
